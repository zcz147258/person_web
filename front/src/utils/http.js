...
axios.defaults.baseURL = 'http://47.101.212.62:9200' // 这是调用数据接口,公共接口url+调用接口名
//获取域名
...
export function getData(url, params) {
  ...
  // 封装axios的post请求
  export function postData(url, params,ret) {
    return new Promise((resolve, reject) => {
  ...
  export function Postdownload(url, params,responsetype) {
    return new Promise((resolve, reject) => {
      axios.post(url, QS.stringify(params),{responseType:'blob'}).then(response => {
          resolve(response);
        })
        .catch(error => {
          reject(error);
